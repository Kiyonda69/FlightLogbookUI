...
- SIM の離着陸回数も記録（離着陸の合計には含めず別に集計。帳票には `(1)` のように記入）
...
- **資格要件チェックリスト シート**: スプレッドシートに社内様式（CAP）と同じレイアウトの「資格要件チェックリスト」シートが作られ、設定タブの入力（所属・社員番号・基準月・実施日・有効期限）と飛行日誌の記録（飛行内容が M12 / M21 / M22 / CACK / M11 / ROUTE / DIT のレグ、および QPR にチェックしたレグ → ROUTE CHK）から、レグの保存や設定の保存のたびに自動更新されます。前回実施日と年度別（4 月始まり、前年度〜3 年後）の実施日、有効期限が埋まります。印刷設定（横向き・1 ページ）はシート側で一度設定してください。
