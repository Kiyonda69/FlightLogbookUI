...
- **入力**: 月日・型式・登録記号・出発/到着・OUT/IN（UTC）・便名を入れ、編成を選んで保存。保存後は到着地が次の出発地に自動セットされます。「復路を作成」で出発/到着を入れ替えた次レグを作れます。
- **編成と飛行時間の自動配分**: 「通常 (2名)」は全時間を自分の DUTY へ。「マルチ (3名)」「ダブル (4名)」はパターン（例 `M2: CA / CA / CO`）と自分の位置を選ぶと、社内の飛行時間配分表 (Rev.2) に従ってブロックタイムを配分します（各項を四捨五入し、飛行時間 (8 項) はその合計）。夜間は実時間を入れれば同じ分数で配分されます。計算結果は「JCAB 全項目を直接編集」で上書きできます。ダブル編成の Pattern 7・8 は資料未確認のため未収録です。
