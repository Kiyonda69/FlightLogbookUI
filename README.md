...
- SIM の離着陸回数も記録（項小計・合計などの合計には表示せず、直近 N 日と 90 日の離着陸経験にだけ実機と合わせて数える。帳票では SIM レグに `(1)` のように記入）
- 月ごとの一覧・編集・削除、項小計 / 前項までの合計 / 合計 の表示
- 年間の月別集計、型式別時間、直近 N 日の時間・離着陸回数（離着陸は SIM を含む）
...
- **QPR**: 入力フォームの「QPR フライト」にチェックして保存すると、一覧に QPR タグが付き、スプレッドシートの「資格要件チェックリスト」シートの **ROUTE CHK 欄**（前回実施日・年度別実施日・基準月）に自動で入ります。集計タブの資格要件にも今年度の回数が出ます。
- **資格要件**: 集計タブに、最終乗務からの経過日数（60 日で復帰訓練）、直近 90 日の離着陸回数（実機 + SIM）、M11/M12/M21/M22 の前回実施日と次回の実施期間（飛行内容の記録から自動）、設定した有効期限（PE / PEA / 航空英語 / 特定操縦技能 / パスポート / VISA）の残日数を表示します。
- **資格要件チェックリスト シート**: スプレッドシートに社内様式（CAP）と同じレイアウトの「資格要件チェックリスト」シートが作られ、設定タブの入力（所属・社員番号・基準月・実施日・有効期限）と飛行日誌の記録（飛行内容が M12 / M21 / M22 / CACK / M11 / ROUTE / DIT のレグ、および QPR にチェックしたレグ → ROUTE CHK）から、レグの保存や設定の保存のたびに自動更新されます。紙の様式と同じく、前回実施日 = 前年度の実施日、その下に今年度〜4 年後（4 月始まり）の実施日と有効期限が埋まり、4 月 1 日に自動で新年度の表に切り替わります（初回だけスプレッドシートのメニュー「飛行日誌 > チェックリストの年度切替を自動化」を実行して権限を承認）。PE / PEA の有効期限は過去分もカンマ区切りで入力できます。印刷設定（横向き・1 ページ）はシート側で一度設定してください。
- **一覧・編集**: 月を選んで確認。行の「編集」で入力タブに読み込み、「削除」で削除。
- **直接編集のチェック**: Flights シートを手で編集すると、壊れた値（時刻に化けた自由欄、`YYYY-MM-DD` でない日付、分でなく `1:30` と書いた時間、ICAO 4 文字でない空港など）のセルが赤くなり、メモに理由が出ます。値は自動では直しません。正しく直せば赤は消えます。シート全体を調べるにはメニュー「飛行日誌 > Flights シートを検査」。
