...
- **入力**: 月日・型式・登録記号・出発/到着・OUT/IN（UTC）・便名を入れ、編成を選んで「キューに追加」または「今すぐ保存」。追加・保存後は到着地が次の出発地に自動セットされます。「復路を作成」で出発/到着を入れ替えた次レグを作れます。便名は自動的に大文字になります。時刻・時間は `0745` のように 4 桁で入れると `07:45` に直ります。
- **まとめて保存**: 「キューに追加」はこの端末のブラウザに一時保存するだけで通信しません（ページを閉じても残ります）。複数レグを入れたら入力タブ下の「未保存レグ」で確認・編集し、「まとめて保存」で 1 回の通信でスプレッドシートに書き込みます（年次シートの更新も 1 回）。1 件でも不正なレグがあると何も書き込まれず、キューは残ります。既存レグの編集もキューに入れられます。
